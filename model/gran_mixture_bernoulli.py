...
__all__ = ['GRANMixtureBernoulli','GRANMixtureBernoulli_nodes_and_edges']
...
class GRANMixtureBernoulli(nn.Module):
  ...
  def _inference(self,
    ...
    #B, C, N_max, _ = F_pad.shape #add definition for feature matrix
    ...
    if self.dimension_reduce:
      node_feat = self.decoder_input(A_pad)  # BCN_max X H #add feature matrix
    else:
      node_feat = A_pad  # BCN_max X N_max #add feature matrixs
  ...
  def forward(self, input_dict):
    ...
    node_attributes = input_dict['attributes'] if 'attributes' in input_dict else None # add feature matrix
...
class GRANMixtureBernoulli_nodes_and_edges(nn.Module):
  """ Graph Recurrent Attention Networks """

  def __init__(self, config):
    super(GRANMixtureBernoulli_nodes_and_edges, self).__init__()
    self.config = config
    self.device = config.device
    self.max_num_nodes = config.model.max_num_nodes
    self.hidden_dim = config.model.hidden_dim
    ...
    self.attribute_encoder = nn.Sequential(
          nn.Linear(self.attribute_dim, self.embedding_dim))

    self.decoder_edges = GNN(
        msg_dim=self.hidden_dim,
        node_state_dim=self.hidden_dim,
        edge_feat_dim=2 * self.att_edge_dim,
        num_prop=self.num_GNN_prop,
  ...
  def _inference(self,
    ...
    print("node_attributes shape: ", node_attributes.shape)
    print("edges: ",edges)
    print("node_idx_gnn: ", node_idx_gnn)
    ...
    print("node_feat[0]")
    print(node_feat[0].shape)
    # Re-order vertex coordinates as (z, y, x).
    print("node_attributes")
    print(node_attributes.shape)#
    node_attributes = node_attributes.view(B * C * N_max, -1)
    print("node_attributes")
    print(node_attributes.shape)#


   # print(node_attributes)
   # node_attributes_permuted = torch.stack([node_attributes[:, 2], node_attributes[:, 1], node_attributes[:, 0]], axis=-1)
 #   print("node_attributes_permuted")
  #  print(node_attributes_permuted.shape)
    #print(node_attributes_permuted)

    node_attributes_encoded = self.attribute_encoder(node_attributes)
    print("node_attributes_encoded")
    print(node_attributes_encoded.shape)
   ...
   #     node_feat, (0, 0, 1, 0), 'constant', value=0.0)  # (BCN_max + 1) X N_max
    ...
    if self.has_rand_feat:
      ...
      att_edge_feat = torch.zeros(edges.shape[0],
                                  2 * self.att_edge_dim).to(node_feat.device)
      # scatter with empty index seems to cause problem on CPU but not on GPU
      print("creating att_edge_feat: ", att_edge_feat.shape)
      print("with edges", edges[:, 0])
      print("and with edges", edges[:, 1])
      print("..............................")
      print(att_idx[[edges[:, 0]]])
      att_edge_feat = att_edge_feat.scatter(1, att_idx[[edges[:, 0]]], 1)
      print("..............................")
      print(att_idx[[edges[:, 1]]] + self.att_edge_dim)
      att_edge_feat = att_edge_feat.scatter(
          1, att_idx[[edges[:, 1]]] + self.att_edge_dim, 1)

    print("att_edge_feat: ", att_edge_feat.shape)
    print(att_edge_feat)

    # GNN inference
    # N.B.: node_feat is shared by multiple subgraphs within the same batch
    print("decoder called with:", node_feat[node_idx_feat].shape)
    node_state_edges = self.decoder_edges(
        node_feat[node_idx_feat], edges, edge_feat=att_edge_feat)
    print("node_state_edges.shape")
    print(node_state_edges.shape)

     
    #feed node_state into predictor for node_attribute prediction
    node_state_attribute = self.decoder_nodes(node_attributes_encoded[node_idx_feat], edges, edge_feat=att_edge_feat)
    print("node_state_attribute.shape")
    print(node_state_attribute.shape)
    
    node_state = torch.cat((node_state_edges, node_state_attribute), 1)
  ...
  def _sampling(self, B):
    ...
    with torch.no_grad():
        ...
        for ii in range(0, N_pad, S):
          ...
          node_state_attributes = self.decoder_nodes(
              node_state_in.view(-1, H), edges, edge_feat=att_edge_feat)
          node_state_attributes = vertex_state_out.view(B, jj, -1)

          
          node_state_edges = self.decoder_edges(
              node_state_in.view(-1, H), edges, edge_feat=att_edge_feat)
          node_state_edges = node_state_out.view(B, jj, -1)

          node_state_out = torch.cat((node_state_attributes, node_state_edges), 1)
  ...
  def forward(self, input_dict):
    ...
    #  B, N, _ = node_attributes.shape	
      ...
      print('GRAN return val:')
      print(A_list)
      return A_list

class GNN_nodes_and_edges(nn.Module):
  ...
  def __init__(self,
               ...
               output_hidden_dim=128,
               graph_output_dim=None):
    super(GNN_nodes_and_edges, self).__init__()
    self.msg_dim = msg_dim
    self.node_state_dim = node_state_dim
    self.edge_feat_dim = edge_feat_dim
    self.num_prop = num_prop
  ...
  def _prop(self, state, edge, edge_feat, node_attr, layer_idx=0):
    ### compute message
    #print("state------------------------------------")
    #print(state.shape)
    #print(state[0])
    ...
    num_edges = edge.shape[0]
    expanded_node_attr = node_attr.expand(num_edges, -1, -1)
    net_input = torch.cat([edge_input, expanded_node_attr], dim=1)
    msg = self.msg_func[layer_idx](net_input)

    ### attention on messages
    if self.has_attention:
      att_weight = self.att_head[layer_idx](net_input)
      msg = msg * att_weight
  ...
  def forward(self, node_feat, edge, edge_feat, node_attr, graph_idx=None):
    """
      N.B.: merge a batch of graphs as a single graph
    ...
    for ii in range(self.num_layer):
      ...
      for jj in range(self.num_prop):
        state = self._prop(state, edge, edge_feat=edge_feat, node_attr=node_attr, layer_idx=ii)
