...
def load_data(type, num_train, num_dev):
    ...
    if type == 'grid_DD':
        ...
        for i in range(0, num_graphs):
            title = "../../artificial_data_gen/data/graph_{}.pickle".format(i)
...
class GRANData_v(object):
  ...
  def _get_graph_data(self, G):
    print("GETTING GRAPH DATA FROM HERE:")
...
if __name__ == "__main__":
    ...
    graphs_train, graphs_dev, graphs_test = load_data('mesh',20,20)
    train_dataset = eval('GRANData_v')(config, graphs_train, tag='train')
    ...
    #node_attribute_list = nx.get_node_attributes(train_dataset.graphs[0],'label')
    #graph_degree =  node_degree_list = [(n, d) for n, d in train_dataset.graphs[0].degree()]

   # print("graph_degree: ") 
   # print(graph_degree)
   # loader = GRANDataLoader(train_dataset, batch_size=1, shuffle=False)
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size= 2, #train_conf.batch_size,
        shuffle=True, #train_conf.shuffle,
        num_workers=2, #train_conf.num_workers,
        collate_fn = train_dataset.collate_fn,
        drop_last=False)
   
    #filename = "" + train_dataset.file_names[0]
    #data = simple_pickle_read(filename)
    #print("data: ")
    #print(data)
